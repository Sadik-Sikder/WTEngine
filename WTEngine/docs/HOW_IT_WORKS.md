...
- **Plumbing:** `__wtFetch` starts a detached `std::thread` running `fetchHttp` (§13) into a `FetchSlot` (a mutex-guarded result - the thread never touches a `JSValue`, since quickjs isn't thread-safe). The promise's resolve/reject functions wait in `FetchStorage`. `Engine::render` calls `pollFetches` every frame, right after `fireDueTimers`; it settles every finished request and runs the resulting promise jobs. A navigation resets `DOMBindingState`, freeing the pending functions; a still-running thread finishes into a slot nothing reads, the same abandon-in-place pattern as `PageLoader`.
...
- **Not supported:** `AbortController`, streaming, CORS (every request is allowed), cookies, `XMLHttpRequest`. Requests aren't limited to a pool the way `ResourceLoader`'s are, so a page firing hundreds at once gets hundreds of threads.
...
| Function | Purpose |
|---|---|
| `fetchPage(url, postBody?)` | HTML text. http(s) via Beast/Asio (`GET` with `Accept: text/html`, or `POST`); anything else is treated as a local file path. Status ≥ 400 → error. Follows up to 10 redirects (as GET after a 301/302/303; a 307/308 keeps the method and body). Decodes UTF-8 (BOM stripped). Reports the final URL after redirects |
| `fetchHttp(HttpRequest)` | For JS `fetch()` (§11): any method, extra headers (overriding the defaults), a body. Returns status, raw body bytes (decompressed), `Content-Type` and the final URL. An HTTP error status is still a completed response (`ok` = a response arrived). A non-http URL is read as a local file (status 200). Same connection pool and redirect handling as `fetchPage` |
