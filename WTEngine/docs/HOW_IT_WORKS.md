...
_Snapshot: latest commit `4bfc5b9` ("Collapse a box to zero height when its own CSS height is exactly 0"), plus this commit's documentation of it._
...
**`@`-rules:** a *bare-media-type* `@media` block - `@media screen{...}`, `@media print{...}`, `@media all{...}`, or no type at all - is evaluated outright (`isSimpleScreenMedia`): `screen`/`all`/none are always true (this engine only ever renders on-screen) and `print` is always false, and its content is parsed as if it weren't wrapped at all (the parse cursor just moves past the block's own opening `{`; the loop's existing stray-`}` handling consumes the closing one once the content's exhausted - no separate recursive parser needed). Every other `@`-rule - `@supports`, `@import`, `@font-face`, `@keyframes`, and any `@media` with a parenthesized feature query (`min-width`, `prefers-color-scheme`, ...) or a comma-separated query list - is still always skipped, never conditionally applied. A feature query needs the viewport size at *match* time (it can change on resize), not parse time, which this parser has no access to at all - a separate, bigger feature, not attempted.
...
- External stylesheets (`<link rel=stylesheet>`) are fetched and applied, but always cascade after every inline `<style>` block regardless of true document order (§10). Bare-media-type `@media` is evaluated (§8); everything else (feature queries, `@supports`, pseudo-classes, attribute selectors, child/sibling combinators) is not. **Confirmed fixed end-to-end on a real page:** Wikipedia's collapsible "Main menu" panel was hidden by `.vector-dropdown-content{opacity:0;height:0;visibility:hidden;...}` (revealed via a `:checked ~` sibling-combinator rule this engine already correctly skips), nested inside a real ~36KB `@media screen{...}` block (confirmed by tracing the parser's byte offsets) - fixed by three independent pieces landing together: `opacity`/`visibility` support, evaluating bare-media-type `@media` instead of always skipping it, and collapsing a box whose own `height` is exactly `0` regardless of its (now-invisible) children's natural size (all §8). Verified by reloading the real page: the dropdown's menu items no longer render inline at the top, replaced by just the real "Main menu"/"Search" labels - the header box is still taller than a real browser's, most likely from other hidden dropdowns in the same header (search suggestions, user menu, language selector) or generic box-model padding differences elsewhere, not from this specific bug; not investigated further.
